...
fs.write("mylogfileVentas.log", "", "w"); // overwrite log file
casper.on("log", function(entry){
    fs.write("mylogfileVentas.log", entry.message + "\n", "a");
});
...
var date = casper.cli.args[3];
// var dc = ''+casper.cli.args[3]+'';

casper.echo(rucEmpresa+'-'+passwordEmpresa+'-'+idEmpresa+'-'+date);
...
casper.then(function () {
  if (casper.exists('form[action="j_security_check"]')) {
    ...
    casper.then(function () {
      casper.thenOpen('https://srienlinea.sri.gob.ec/tuportal-internet/menusFavoritos.jspa?redireccion=60&idGrupo=58', function () {
        this.userAgent('Mozilla/4.0 (compatible; MSIE 6.0; Windows NT 5.1)');
        var sendCa = '';
        var random = Math.floor((Math.random() * 1000) + 1);
        casper.then(function () {
          casper.thenClick(x('//*[@class="sri-listaLinks"][2]//a[@href="#"]'), function () {
            casper.echo('Click On Link...');
            casper.then(function () {
              casper.wait(2000, function () {
                this.captureSelector('captures/captcha'+random+'.png', '#elemento-captcha');
                casper.then(function () {
                  casper.then(function () {
                    url = 'https://www.practisis.net/casper.image.auto.php?idemp='+casper.cli.args[2]+'&imagen='+random+'.png&error=false';
                    casper.echo(url)
                  ...
                  casper.then(function () {
                    casper.wait(20000, function () {
                      if (sendCa != 'ERROR') {
                        casper.then(function () {
                          url = 'https://www.practisis.net/casper.captcha.auto.php?idemp='+casper.cli.args[2]+'&sendXa='+sendCa+'&error=false';
                          casper.echo(url)
            ...
            casper.then(function () {
              if (sendCa != '' && sendCa > 0 && sendCa != null) {
                casper.echo('sendCa PASAMOSSS')
                sendCa = sendCa - 1;
                var date_to_fill = casper.cli.args[3];
                var document_to_fill = 6;
                var fill_date = casper.evaluate(function(date, document_comprobante) {
                  var d = document.getElementById('frmPrincipal:calendarFechaDesde_input');
                  var doc = document.getElementById('frmPrincipal:cmbTipoComprobante');
                  d.value = date;
                  doc.value = document_comprobante;
                  return [d.value, doc.value];
                }, {
                    date: date_to_fill,
                    document_comprobante: document_to_fill
                });
                casper.then(function () {
                  casper.click('#visualCaptcha-img-'+sendCa+'');
                })
                casper.then(function () {
                  casper.capture('captures/formfill.png')
                })
                  ...
                  casper.then(function () {
                    ...
                    if (pagination[0] == 1) {
                      casper.each(respuesta[0], function (cas, value) {
                        ...
                        casper.echo(correctDate+'ESTA NO INGRESO');
                      })
                    }else{
                      for (var i = 1; i < pagination[0]; i++) {
            ...
            casper.then(function () {
              casper.capture('captures/prueba1.png')
            })
          })
        })
      })
